    ...
    <div className="animate-fade-in pb-20 max-w-2xl mx-auto space-y-8 flex flex-col items-center">
      <Card className="w-full p-8 flex flex-col items-center text-center">
        <div className="relative mb-6">
          <Avatar fallback={user?.name ? user.name.charAt(0).toUpperCase() : (role === 'student' ? 'S' : 'R')} size="lg" className="w-24 h-24 text-2xl bg-color-accent text-white border-4 border-color-surface" />
        ...
        <h1 className="text-2xl font-bold font-serif">{user?.name || (role === 'student' ? 'Student' : 'Teacher')}</h1>
        <p className="text-color-muted mt-1 uppercase text-xs tracking-widest">{role}</p>
